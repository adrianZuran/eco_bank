<x-admin-layout>
    ...
    <div class="max-w-3xl mx-auto">
        ...
        <div class="bg-white border border-gray-100 rounded-2xl shadow-sm overflow-hidden">
            <form action="{{ route('admin.missions.store') }}" method="POST" class="p-8">
                @csrf

                <div class="space-y-6">
                    <div>
                        <label for="title" class="block text-sm font-bold text-gray-700 mb-2">Judul Misi <span class="text-red-500">*</span></label>
                        <input type="text" name="title" id="title" value="{{ old('title') }}" required class="w-full rounded-xl border-gray-300 focus:border-[#5C8D3A] focus:ring focus:ring-[#5C8D3A]/20 transition-shadow">
                        @error('title')
                    ...
                    <div>
                        ...
                        <textarea name="description" id="description" rows="3" class="w-full rounded-xl border-gray-300 focus:border-[#5C8D3A] focus:ring focus:ring-[#5C8D3A]/20 transition-shadow">{{ old('description') }}</textarea>
                        @error('description')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input type="number" name="reward_points" id="reward_points" value="{{ old('reward_points', 0) }}" min="0" required class="w-full rounded-xl border-gray-300 focus:border-[#5C8D3A] focus:ring focus:ring-[#5C8D3A]/20 transition-shadow">
                        @error('reward_points')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="flex items-center">
                        <input type="checkbox" name="is_active" id="is_active" value="1" {{ old('is_active', true) ? 'checked' : '' }} class="rounded border-gray-300 text-[#5C8D3A] shadow-sm focus:ring-[#5C8D3A] w-5 h-5">
